            ...
            for (const [idx, eventNameAndFilter] of eventNameAndFilterList.entries()) {
                ...
                try {
                    const callsAndParams = [
                        getPastEventsForBatchRequest(contract, eventName, {
                            filter: filter,
                    ...
                    if ((timestampStop !== null) && (idx === 0)) {
                        ...
                        callsAndParams.push([getBlockForBatchRequest(contract), blockRange.fromBlock]);
                    }
                    const results = await makeBatchRequestPromise(new contract.BatchRequest(), callsAndParams);
